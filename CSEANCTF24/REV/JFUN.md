...
public class CSEAN {
    public static String hashCharacter(char character) {
        try {
            MessageDigest digest = MessageDigest.getInstance("SHA-256");

            byte[] hash = digest.digest(Character.toString(character).getBytes(StandardCharsets.UTF_8));
    ...
    private static String conv(byte[] hash) {
    ...
    public static void main(String[] args) {
        String text = "FAKE_FLAG";

        for (char chr : text.toCharArray()) {
            String hashed = hashCharacter(chr);
...
// 4b227777d4dd1fc61c6f884f48641d02b4d121d3fd328cb08b5531fcacdabf8a
```
We were given a java source code

Seeing the code takes in a string and hashes it, so we were given different hashes and one on the hashes is the flag, so we just have to reverse the hashing operation on the hashes to get which one is the flag, so I used the script below
